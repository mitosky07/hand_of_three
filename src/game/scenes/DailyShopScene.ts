...
export class DailyShopScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(140, 125, `SEED ${state.date} · ROUND ${state.round} · SAME OFFERS FOR EVERYONE`, pixelText(7, "#7fa98a")).setOrigin(0, .5);
  ...
  private renderOffer(offer: DailyOffer, index: number, wallet: Phaser.GameObjects.Text, message: Phaser.GameObjects.Text) {
    ...
    this.add.text(x - 200, y - 18, offer.description.toUpperCase(), { ...pixelText(7, "#c9bea0"), wordWrap: { width: 390 }, lineSpacing: 7 }).setOrigin(0, 0);
