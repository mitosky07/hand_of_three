...
export class CosmeticsScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(130, 92, "TABLE LOCKER", pixelText(24, "#e0ad4f")).setOrigin(0, .5);
    this.add.text(130, 128, "COSMETIC REWARDS · NO GAMEPLAY ADVANTAGE", pixelText(10, "#8fd0c9")).setOrigin(0, .5);
    ...
    FELT_COSMETICS.forEach((felt, index) => {
      ...
      this.add.text(320, y + 13, unlocked ? felt.description.toUpperCase() : `LOCKED · ${felt.winsRequired} WINS REQUIRED`, pixelText(9, unlocked ? "#c2cbd0" : "#d9785f")).setOrigin(0, .5);
    ...
    this.add.text(640, 570, `PORTRAIT GALLERY · ${profile.unlockedPortraits.map((id) => id.replace("_PORTRAIT", "")).join(" · ")}`, pixelText(9, "#8fd0c9")).setOrigin(.5);
